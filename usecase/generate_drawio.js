...
let diagramIdCounter = 1;
for (const diag of diagrams) {
    const pageId = "page" + diagramIdCounter;
    ...
    for (let i = 0; i < diag.messages.length; i++) {
        ...
        if (msg.from === msg.to) {
            ...
            xml += '            <Array as="points">\n';
            xml += '              <Point x="' + (sx + 40) + '" y="' + currentY + '" />\n';
            xml += '              <Point x="' + (sx + 40) + '" y="' + (currentY + 20) + '" />\n';
            xml += '            </Array>\n';
            xml += '          </mxGeometry>\n';
            xml += '        </mxCell>\n';
            currentY += stepY + 15; 
...
const outputPath = path.join(__dirname, 'sequence_diagrams.drawio');
